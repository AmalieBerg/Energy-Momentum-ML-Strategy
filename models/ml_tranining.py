"""
ML Training - Merge Regime Labels and Train Models
Handles wide-format data where all assets are in same rows
"""
...
# Assets we want to train models for
ASSETS = [
    'WTI_Crude',
    'Brent_Crude',
    'Natural_Gas',
...
def load_and_merge_data():
    ...
    if not os.path.exists(regime_path):
        ...
        print("You need to run regime_detector.py first to create regime labels")
        print("\nAttempting to continue without regime labels...")
        print("Will create a simple trend-based target instead")
        
        # Create simple target based on WTI momentum as fallback
        df_features['regime'] = pd.cut(
            df_features['WTI_Crude_momentum_21d'], 
            bins=3, 
            labels=[0, 1, 2]
        ).astype(int)
        print("\nCreated simple 3-regime target based on WTI_Crude_momentum_21d")
        
        return df_features
    
    # Load and merge regime labels
...
def prepare_asset_data(df, asset_name):
    ...
    y : pd.Series
        Target (regime) for this asset
    """
    print(f"\n" + "-"*70)
    print(f"PREPARING DATA FOR: {asset_name}")
    print("-"*70)
    
    # Find regime column for this asset
    regime_col = None
    possible_names = [
        f'{asset_name}_regime',
        f'{asset_name.lower()}_regime',
        'regime',  # If there's a single regime column for all assets
    ]
    ...
    # Include: asset-specific features + market features (VIX, DXY, yield curve)
...
def train_and_evaluate_model(X, y, model, model_name, asset):
    ...
    for fold, (train_idx, test_idx) in enumerate(tscv.split(X), 1):
        ...
        print(f"  Fold {fold}: Accuracy={cv_scores['accuracy'][-1]:.4f}, F1={cv_scores['f1'][-1]:.4f}")
...
def train_all_models():
    ...
    for asset in ASSETS:
        ...
        try:
            ...
            X, y = prepare_asset_data(df, asset)
            
            # Train each model
            for model_name, model in models.items():
                try:
                    result = train_and_evaluate_model(X, y, model, model_name, asset)
