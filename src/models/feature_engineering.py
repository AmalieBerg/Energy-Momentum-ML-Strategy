...
Combines technical indicators with GARCH volatility expertise
"""
import sys
...
from src.models.garch_model import EnergyGARCHModel
import pandas as pd
import numpy as np
...
class MarketFeatureEngineer:
    ...
    def create_momentum_features(self, prices: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"   ✅ Created {len([c for c in momentum_features.columns if 'momentum' in c])} momentum features")
        return momentum_features
    
    def create_volatility_features(self, returns: pd.DataFrame) -> pd.DataFrame:
        """Create volatility-based features using GARCH"""
        logger.info("🔥 Creating volatility features with GARCH models...")
        ...
        for asset in returns.columns:
            ...
            try:
                ...
                garch_vol = garch_vol.reindex(returns.index, method='nearest')
                ...
                logger.warning(f"     ⚠️ GARCH failed for {asset}: {e}")
                # Fallback to simple realized volatility
                vol_features[f"{asset}_garch_vol"] = returns[asset].rolling(21).std()
        
        logger.info(f"   ✅ Created {len([c for c in vol_features.columns if 'vol' in c])} volatility features")
        return vol_features
    ...
    def create_market_structure_features(self, prices: pd.DataFrame, returns: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"   ✅ Created {len(structure_features.columns)} market structure features")
        return structure_features
    
    def get_external_market_data(self) -> pd.DataFrame:
        """Download external market indicators (VIX, Treasury rates, etc.)"""
        logger.info("🌍 Downloading external market indicators...")
        ...
        for ticker, name in external_tickers.items():
            try:
                ...
                if len(data) > 0 and len(data.columns) > 0:
                    ...
                    logger.info(f"   ✅ {name}: {len(external_data.get(name, []))} observations")
                else:
        ...
        if external_data:
            ...
            if 'VIX' in external_df.columns:
                ...
                features['VIX_vs_20'] = (external_df['VIX'] > 20).astype(int)
            ...
            logger.info(f"   ✅ Created {len(features.columns)} external market features")
            return features
        else:
    ...
    def create_all_features(self, prices: pd.DataFrame, returns: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("🧠 Creating complete feature set for ML regime detection...")
        ...
        # Clean the data (fixed deprecated methods)
        ...
        logger.info(f"   📈 Sample size: {len(all_features)} observations")
        
        # Save features for later use
        features_file = PROCESSED_PATH / 'ml_features.csv'
...
if __name__ == "__main__":
    ...
    # Load data
    prices = pd.read_csv(DATA_PATH / 'energy_prices.csv', index_col=0, parse_dates=True)
    returns = pd.read_csv(DATA_PATH / 'energy_log_returns.csv', index_col=0, parse_dates=True)
    ...
    logger.info(f"   Memory usage: {features.memory_usage().sum() / 1024**2:.1f} MB")
