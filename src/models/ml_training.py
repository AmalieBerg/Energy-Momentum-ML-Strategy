...
Trains multiple models for regime prediction
"""
import sys
...
def load_and_merge_data():
    ...
    logger.info(f"Loaded {len(df_features)} rows, {len(df_features.columns)} columns")
    ...
    if not regime_path.exists():
        ...
        logger.warning("Creating simple 3-regime target based on momentum")
    ...
    logger.info(f"Loaded {len(df_regimes)} regime labels")
    ...
    if missing.sum() > 0:
        ...
        logger.warning("Dropping rows with missing regimes")
        df_merged = df_merged.dropna(subset=regime_cols)
    
    logger.info(f"Final data: {len(df_merged)} rows with regime labels")
    
    return df_merged


def prepare_asset_data(df: pd.DataFrame, asset_name: str) -> tuple:
    """Prepare features for a specific asset"""
    logger.info(f"\n" + "-"*70)
    logger.info(f"PREPARING DATA FOR: {asset_name}")
    logger.info("-"*70)
    ...
    if regime_col is None:
        ...
        if len(regime_cols) > 0:
            ...
            logger.info(f"Using generic regime column: {regime_col}")
        else:
            raise ValueError(f"No regime column found for {asset_name}")
    
    logger.info(f"Target variable: {regime_col}")
    ...
    logger.info(f"Number of unique classes: {n_classes}")
    ...
    logger.info(f"\nFeatures prepared:")
    logger.info(f"  Asset-specific features: {len(asset_features)}")
    logger.info(f"  Market features: {len([col for col in market_features if col in df_valid.columns])}")
...
def train_and_evaluate_model(X: pd.DataFrame, y: pd.Series, model, 
                             model_name: str, asset: str, n_classes: int) -> dict:
    """Train and evaluate a single model"""
    logger.info(f"\n" + "-"*70)
    logger.info(f"Training {model_name} for {asset}")
    logger.info("-"*70)
    ...
    logger.info(f"Original classes: {sorted(y.unique())}")
    logger.info(f"Encoded classes: {sorted(np.unique(y_encoded))}")
    ...
    for fold, (train_idx, test_idx) in enumerate(tscv.split(X), 1):
        ...
        logger.info(f"  Fold {fold}: Train classes={train_classes}, Test classes={test_classes}")
    ...
    encoder_path = MODEL_PATH / encoder_filename
...
def train_all_models():
    ...
    if LIGHTGBM_AVAILABLE:
        logger.info("\nUsing LightGBM (better handling of class labels)")
        logger.info("Models: Random Forest, Gradient Boosting, LightGBM, Logistic Regression")
    else:
        logger.info("\nLightGBM not available. Using 3 models only.")
        logger.info("To install LightGBM: pip install lightgbm")
    ...
    if len(all_results) > 0:
        ...
        results_path = RESULTS_PATH / 'model_comparison.csv'
        ...
        logger.info("\n" + "="*70)
        logger.info("BEST MODELS BY ASSET")
        logger.info("="*70)
        ...
        for asset in ASSETS:
            ...
            if len(asset_results) > 0:
                ...
                logger.info(f"  F1 Score: {best['avg_f1']:.4f}")
                logger.info(f"  Accuracy: {best['avg_accuracy']:.4f}")
